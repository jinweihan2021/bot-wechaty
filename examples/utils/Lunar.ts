...
export class Lunar {
   ...
   */
  static monthDays(y:number, m:number) {
    // @ts-ignore
   ...
   * @param objDate
   */
  static calc(objDate:Date): LunarDateInfo {
    //年==============================
    let i, leap, temp = 0;
    //与1900-1-31间隔天数
    ...
    const cYear = this.cYear(year)
    const cMonth = this.cMonth(i)
    const cDay = this.cDay(offset + 1)
    ...
    return {
      ...
      cMonth: cMonth,
      cDay: cDay,
      str: cYear + "年" + (isLeap ? "闰" : "") + cMonth + cDay
    };
  }

  static cYear(y:number) {
